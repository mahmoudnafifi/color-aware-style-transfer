# -*- coding: utf-8 -*-
"""color_aware_style_transfer.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1VNhyI_Ion1Qe8SkA8OIszz_5KGkUWlso
"""
...
STYLE_IMAGE = "./data/images/styleD.jpg"

CONTENT_IMAGE = "./data/images/flower.jpg"
...
COLOR_PALETTE_EXTRACTION = 'python' #'python' 'matlab'
...
def image_loader(image_name, K=16):
    ...
    if COLOR_PALETTE_EXTRACTION == 'python':
        palette_selection = PaletteSelection(color_coordinates,
                                            color_densities, rgb_colors,
                                            num_colors=K, sigma=70.0)
        colors = palette_selection._palette_colors

    elif COLOR_PALETTE_EXTRACTION == 'matlab':
        os.system(f'extract_theme.exe {image_name} {K} 20')

        colors = scipy.io.loadmat('color_palette.mat')['color_palette']
...
def image_loader_with_color_transfer(image_name, target_image, K=16):
    image = Image.open(image_name)
    # compute color palette
    img_array = np.array(image)
    target_image = target_image.cpu().clone()  # we clone the tensor to not do changes on it
    target_image = target_image.squeeze(0)      # remove the fake batch dimension
    target_image = unloader(target_image)
    target_array = np.array(target_image)

    img_array_lab = Lab2rgb(img_array)
    target_array_lab = Lab2rgb(target_array)


    for c in range(3):
      img_array_lab[:, :, c] = (((
          img_array_lab[:, :, c] - np.mean(img_array_lab[:, :, c])) * (
              np.std(target_array_lab[:, :, c])/np.std(img_array_lab[:, :, c])
              )) + np.mean(target_array_lab[:, :, c]))


    img_array = rgb2Lab(img_array_lab)
...
# content_img, content_palette = image_loader_with_color_transfer(
#     CONTENT_IMAGE, style_img, PALETTE_SIZE)
...
style_masks = mask_generator(style_img)

content_masks = mask_generator(content_img)

# ##show masks
directory = 'D:/style_transfer/figures/temp'
save_image(final_color_palette_vis.squeeze(0),
           f'{directory}/final_palette.png')
save_image(content_img_color_palette_vis.squeeze(0),
           f'{directory}/content_palette.png')
save_image(style_img_color_palette_vis.squeeze(0),
           f'{directory}/style_palette.png')
...
if SHOW_MASKS:
    for i in range(final_palette.shape[0]):
        ...
        save_image(style_masks[:, i, :, :].squeeze(0),
                   f'{directory}/style_mask_{i}.png')
        save_image(content_masks[:, i, :, :].squeeze(0),
                   f'{directory}/content_mask_{i}.png')
...
def get_style_model_and_losses(cnn, normalization_mean, 
    ...
    #return model, style_losses, content_losses, color_aware_style_losses
...
def run_style_transfer(cnn, normalization_mean, normalization_std,
    ...
    # model, style_losses, content_losses, color_aware_losses = ( 
    ...
    while run[0] <= num_steps:
        ...
        def closure():
            ...
            #loss = style_score + content_score + color_aware_score 
            loss = content_score + color_aware_score 
            #loss = style_score + content_score
            #loss = content_score + histogram_score
            ...
            if run[0] % 50 == 0:
                ...
                # print('Style Loss : {:4f} Content Loss: {:4f} Color-Aware Loss: {:4f}'.format(
                #     style_score.item(), content_score.item(),
                #     color_aware_score.item()))
                print('Content Loss: {:4f} Color-Aware Loss: {:4f}'.format(
                    content_score.item(), color_aware_score.item()))
            

            #return style_score + content_score + color_aware_score 
...
save_image(output.squeeze(0), 'data/images/output.png')